...
class AdminArticleController extends Controller
{
    ...
    public function store(RequestArticle $requestArticle)
    {
        ...
        return redirect()->back()->with('success', 'Hoàn thành thêm mới bài viết!!!');
    }
    ...
    public function update(RequestArticle $requestArticle, $id)
    {
        ...
        return redirect()->back()->with('success', 'Hoàn thành cập nhật bài viết!!!');
    }
    ...
    public function action(Request $request, $action, $id)
    {
        if ($action) {
            ...
            switch ($action)
            {
                case 'delete':
                    $article->delete();
                    $message = 'Hoàn thành xóa bài viết!!!';
                    break;
                case 'active':
                    $article->a_active = $article->a_active ? 0 : 1;
                    $article->save();
                    $message = 'Hoàn thành hay đổi trạng thái!!!';
                    break;
                case 'hot':
                    $article->a_hot = $article->a_hot ? 0 : 1;
                    $article->save();
                    $message = 'Hoàn thành thay đổi nổi bật!!!';
                    break;
            }
        }
