...
class AdminProductController extends Controller
{
    ...
    public function store(RequestProduct $requestProduct)
    {
        ...
        return redirect()->back()->with('success', 'Hoàn thành thêm mới sản phẩm!!!');
    }
    ...
    public function update(RequestProduct $requestProduct, $id)
    {
        ...
        return redirect()->back()->with('success', 'Hoàn thành cập nhật sản phẩm!!!');
    }
        ...
        if ($action) {
            ...
            switch ($action)
            {
                case 'delete':
                    $product->delete();
                    $rating->delete();
                    $messages = 'Hoàn thành xóa sản phẩm!!!';
                    break;
                case 'active':
                    $product->pro_active = $product->pro_active ? 0 : 1;
                    $product->save();
                    $messages = 'Hoàn thành thay đổi trạng thái sản phẩm!!!';
                    break;
                case 'hot':
                    $product->pro_hot = $product->pro_hot ? 0 : 1;
                    $product->save();
                    $messages = 'Hoàn thành thay đổi nổi bật sản phẩm!!!';
                    break;
            }
        }
