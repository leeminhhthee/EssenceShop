...
class AdminCategoryController extends Controller
{
    ...
    public function store(RequestCategory $requestCategory)
    {
        ...
        return redirect()->back()->with('success', 'Hoàn thành thêm mới danh mục!!!');
    }
    ...
    public function update(RequestCategory $requestCategory, $id)
    {
        ...
        return redirect()->back()->with('success', 'Hoàn thành cập nhật danh mục!!!');
    }
        ...
        if ($action) {
            ...
            switch ($action)
            {
                case 'delete':
                    ...
                    //xoa cate
                    $category->delete();
                    $messages = "Hoàn thành xóa danh mục!!!";
                    break;
                case 'active':
                    $category->c_active = $category->c_active ? 0 : 1;
                    $messages = "Hoàn thành thay đổi trạng thái danh mục!!!";
                    $category->save();
                    break;
                case 'home':
                    $category->c_home = $category->c_home ? 0 : 1;
                    $messages = "Hoàn thành thay đổi hiển/ẩn trang chủ danh mục !!!";
                    $category->save();
                    break;
            }
        }
